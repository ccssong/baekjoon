...
long long find(vector<long long> a){
    ...
    long long size = 0, l = 0, min = 10000, mini = 0, mini2 = 0, sum = 0, sum2=0;
    bool flag = false;
    ...
    vector<long long> left(a.begin(), a.begin() + mini);
    vector<long long> right(a.begin() + mini, a.end());

    sum += find(left);
    sum += find(right);

    if(flag){
        vector<long long> left2(a.begin(), a.begin() + mini2);
        vector<long long> right2(a.begin() + mini2, a.end());
        sum2 += find(left2);
        sum2 += find(right2);
    }else{
        sum2 = sum + 1;
...
void cut(vector<long long> a){
    result.resize(N-1);
    for(int i = 0; i < a.size()-1; i++){
        vector<long long> left(a.begin(), a.begin() + i+1);
        vector<long long> right(a.begin() + i+1, a.end());
        result[i] += find(left);
        result[i] += find(right);
